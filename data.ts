import { Serie } from './serie.js';
export const series: Serie[] = [
  new Serie(1, "Breaking Bad", "AMC", 5,
    "Set and filmed in Albuquerque, New Mexico, the series tells the story of Walter White, a struggling and depressed high school chemistry teacher who is diagnosed with lung cancer.",
    "https://upload.wikimedia.org/wikipedia/en/6/61/Breaking_Bad_title_card.png",
    "https://www.amc.com/shows/breaking-bad"),

  new Serie(2, "Orange Is the New Black", "Netflix", 6,
    "Comedy-drama series about life in a women's prison.",
    "https://upload.wikimedia.org/wikipedia/en/6/6e/Orange_Is_the_New_Black_title_card.png",
    "https://www.netflix.com/title/70242311"),

  new Serie(3, "Game of Thrones", "HBO", 7,
    "Fantasy drama based on George R. R. Martin's novels.",
    "https://upload.wikimedia.org/wikipedia/en/d/d8/Game_of_Thrones_title_card.jpg",
    "https://www.hbo.com/game-of-thrones"),

  new Serie(4, "The Big Bang Theory", "CBS", 12,
    "Sitcom about socially awkward scientists and their friends.",
    "https://upload.wikimedia.org/wikipedia/en/6/6d/The_Big_Bang_Theory_title_card.png",
    "https://www.cbs.com/shows/big_bang_theory/"),

  new Serie(5, "Sherlock", "BBC", 4,
    "Modern adaptation of Arthur Conan Doyle's detective stories.",
    "https://upload.wikimedia.org/wikipedia/en/b/b9/Sherlock_titlecard.jpg",
    "https://www.bbc.co.uk/programmes/b018ttws"),

  new Serie(6, "A Very English Scandal", "BBC", 2,
    "Drama based on the true story of Jeremy Thorpe.",
    "https://upload.wikimedia.org/wikipedia/en/2/2e/A_Very_English_Scandal_titlecard.jpg",
    "https://www.bbc.co.uk/programmes/p065smy4")
];
